    ...
    return (
        <div>
            <Disclosure as="nav">
                ...
                <div className="fixed top-0 z-20 h-screen p-6 duration-200 ease-out delay-150 bg-white -left-96 lg:left-0 lg:w-60 peer-focus:left-0 peer:transition md:w-48">
                    <div className="flex flex-col justify-start item-center">
                        ...
                        <div className="pb-4 my-4 border-b border-gray-100">
                            {/* Dashboard */}
                            <NavItem icon={<MdOutlineSpaceDashboard />} label="Dashboard" />
                            {/* Integration */}
                            <NavItem icon={<MdOutlineAgriculture />} label="Services" />
                            {/* Comments */}
                            <NavItem icon={<FaRegComments />} label="Ask Experts" />
                            {/* Analytics */}
                            <NavItem icon={<MdOutlineAnalytics />} label="Reports" />
                            {/* Messages */}
                            <NavItem icon={<BiCalendarEdit />} label="Plan Farm Activity" />
                        ...
                        {/* Logout */}
                        <div className="my-6">
                            <NavItem icon={<MdOutlineLogout />} label="Logout" />
                        </div>
                    </div>
                </div>
            </Disclosure>
...
function NavItem({ icon, label }) {
    return (
        <div className="flex items-center justify-start gap-4 p-2 pl-5 m-auto mb-2 text-black rounded-md cursor-pointer hover:bg-green-600 group hover:shadow-lg hover:text-white">
            {icon}
            <h3 className="text-base font-semibold text-black group-hover:text-white">{label}</h3>
        </div>
    );
}
